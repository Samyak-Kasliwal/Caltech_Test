...
class PoseEncoder(nn.Module):
    def __init__(self, config: EmbeddingConfig, joint_encoder: JointEncoder) -> None:
        super().__init__()
        self.joint_encoder = joint_encoder
        self.pose_mlp = build_mlp(
            7 * config.joint_embedding_dim,
...
class MouseEncoder(nn.Module):
    def __init__(self, data_config: DataConfig, embedding_config: EmbeddingConfig) -> None:
        ...
        self.pose_encoder = PoseEncoder(embedding_config, shared_joint_encoder)
        self.velocity_encoder = VelocityEncoder(embedding_config)
    ...
    def forward(
        ...
        mouse_features = torch.cat(
            [pose_embedding, velocity_embedding, behavior_embedding], dim=-1
        )
...
class AsymmetricEmbeddingModule(nn.Module):
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        a_embedding = self.mouse_encoder(
            batch["a_xy"], batch["a_velocity"], batch["a_behavior"]
        )
        b_embedding = self.mouse_encoder(
            batch["b_xy"], batch["b_velocity"], batch["b_behavior"]
        )
